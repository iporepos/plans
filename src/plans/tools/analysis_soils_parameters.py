...
"""
Tool for spatialising soils parameters onto the study area grid.
"""
...
# =======================================================================
from ..datasets import Soils, AOI, RasterCollection, SciRaster
...
class LocalTool(Tool):
    ...
    def __init__(self, folder_output):
        ...
        name = "plans.tools.analysis_soils_parameters"
    ...
    def load_data(self):
        ...
        df_parameters = parse_spatial_parameters(
            title="Soils Attributes", file_parameters=self.file_parameters
        )

        # load maps
        # ---------------------------------------------------------------
        msg = f"{self.label_loading} >>> soils map ..."
        self.logger.info(msg)
        soils = Soils(name=self.label)
        soils.load_data(file_data=self.file_soils, file_table=self.file_attributes)
    ...
    def process_data(self):
        ...
        for row in df_values.to_dict(orient="records"):
            ...
            # downscale
            # -----------------------------------------------------------
            msg = f"{self.label_processing} >>> {self.label} downscaling {row['name']} ..."
            self.logger.info(msg)
            ...
            # set-up
            # -----------------------------------------------------------
            msg = f"{self.label_processing} >>> {self.label} building raster {row['name']} ..."
            self.logger.info(msg)
